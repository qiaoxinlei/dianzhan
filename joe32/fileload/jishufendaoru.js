...
router.post("jishufen", async (ctx) => {
...
if(shuzi !=''){
  ...
  .where({bao:num2})
  .update({jishufen:file.name})
}
else{
  let shuzi1= await ceshi('fileload')
  .insert({jishufen:file.name,bao:num2})
...
if(baobao!=num2){
  ...
  let chaxun = await ceshi('jishufen')
  ...
  for(var p=0;p<newshuju.length;p++){
      if(chaxun !=''){
           console.log(777)
           newshangwu = await ceshi('jishufen')
